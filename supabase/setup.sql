...
-- ── Added: devices RLS policies (missing from base schema) ───────────────
-- Lets a JUDGE see devices for their own cases, OPERATIONAL for assigned cases.
CREATE POLICY "devices_judge" ON devices FOR SELECT
  USING (
    current_user_role() = 'JUDGE' AND EXISTS (
      SELECT 1 FROM cases c WHERE c.id = devices.case_id
        AND c.judge_id = (SELECT id FROM users WHERE auth_id = auth.uid())
    )
  );

CREATE POLICY "devices_operational_assigned" ON devices FOR SELECT
  USING (
