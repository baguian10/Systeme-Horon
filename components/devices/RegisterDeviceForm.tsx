...
export default function RegisterDeviceForm() {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold">
  ...
  return (
    <div className="flex items-end gap-2 flex-wrap bg-gray-50 border border-gray-200 rounded-xl p-3">
      ...
      <button onClick={submit} disabled={busy} className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold disabled:opacity-40">
        {busy ? '…' : 'Ajouter'}
      </button>
